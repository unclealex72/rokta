...
class LeagueFactoryImplSpec extends Specification with Mockito {
  ...
  def mocked[B](block: LeagueFactoryImpl => GapCalculator => B) = {
    val gapCalculator = mock[GapCalculator]
  ...
  "A non-current league" should {
    "not be decorated" in mocked { (leagueFactory: LeagueFactoryImpl) => (gapCalculator: GapCalculator) =>
      def snapshots = 
        on(
  ...
  "Decorating a league with who is currently playing and who is currently not" should {
    "mark playing players' results as current" in mocked { 
      (leagueFactory: LeagueFactoryImpl) => (gapCalculator: GapCalculator) =>
