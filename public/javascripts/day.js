...
day.service('DateSerialiser', function() {
  ...
  var service = {
	serialise: function(date) {
	  var args = [{length: 4, value: date.year}, {length: 2, value: date.month}, {length: 2, value: date.day}];
...
day.service('Sanitiser', function() {
  return {
    sanitise: function(obj) {
      return _.omit(obj, function(value) { return !_.isNumber(value); });
    } 	  
  };
});

day.filter('roktaYear', ['Sanitiser', function(Sanitiser) {
  return function(day) {
	if (!day) {
	  return;
	}
...
day.filter('roktaMonth', ['Sanitiser', '$filter', function(Sanitiser, $filter) {
...
day.filter('roktaDay', ['DateSuffix', 'Sanitiser', '$sce', '$filter', function(DateSuffix, Sanitiser, $sce, $filter) {
  ...
  return function(day, excludeMonth, excludeYear) {
	...
	if (_.isDate(day)) {
	  day = {year: day.getFullYear(), month: day.getMonth() + 1, day: day.getDate()};
	}
	day = Sanitiser.sanitise(day);
	var output = 
