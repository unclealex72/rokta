...
class StatsFactoryImpl(
  ...
  _handCountsFactory: Option[HandCountsFactory] = injected,
  ...
  val handCountsFactory = injectIfMissing(_handCountsFactory)
  ...
  def apply(contiguousGameFilter: ContiguousGameFilter): Stats[Game] = {
    ...
    val handCounts = handCountsFactory(games)
    ...
    Stats(
      ...
      league, snapshots, headToHeads, handCounts, streaks, exemptPlayer, lastGame, numberOfGamesToday)
  }
