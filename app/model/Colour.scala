...
object ColourGroup extends PersistableEnumeration[ColourGroup] {
  ...
  /**
   * A base class for persistable enumerations that ensures a persistable token is supplied through a constructor
   * and is thus available to build the internal valueMap.
   */
  abstract class Persistable(val persistableToken: String, val description: String)
