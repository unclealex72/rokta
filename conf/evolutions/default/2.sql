...
ALTER FUNCTION week(timestamp without time zone)
  OWNER TO rokta;
CREATE OR REPLACE FUNCTION day("when" timestamp without time zone)
  RETURNS integer AS
'select cast(extract(day from $1) as integer);;'
  LANGUAGE sql VOLATILE
  COST 100;
ALTER FUNCTION day(timestamp without time zone)
  OWNER TO rokta;

# --- !Downs
DROP FUNCTION year("when" timestamp without time zone);
DROP FUNCTION month("when" timestamp without time zone);
DROP FUNCTION day("when" timestamp without time zone);
